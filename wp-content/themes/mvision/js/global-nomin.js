...
jQuery(function($) {
	...
	if(_ismobile){
		...
		if(!$('body').hasClass('single-event')) $('video').remove(); 
	}

	_functions.initIsotope = function(){
		if( $('.grid').length ){
	        $grid = $('.grid').isotope({
	...
	//--height block--//
	_functions.blockHeight = function(){
        var winW = $(window).width();
        if( $('.smileContentHeight').length ){
            var col = ( winW > 1280 ? 3 : 2 );
            if( winW > 767 ){
                $('.smileContentHeight').each(function(){
                    var highestBox = ['0'],
                        count = 0,
                        count2 = 0,
                        size = $('.smileContentWrapper', this).closest('.grid-item:visible').length;
                    $('.smileContentWrapper', this).each(function(i){
                        $(this).css('height', 'auto');
                        if( $(this).height() > highestBox[count] ) highestBox[count] = $(this).height();
                        if( (i+1)%col == 0 && (i+1) != size){
                            highestBox.push('0');
                            count++;
                        }
                    });
                    $('.smileContentWrapper', this).each(function(index){
                        if( $(this).height() != highestBox[count2] ) $(this).css('height', highestBox[count2]);
                        if( (index+1)%col == 0 && (index+1) != size) count2++;
                    });
                });
            } else {
                $('.smileContentHeight .smileContentWrapper').css('height', 'auto');
            }
        }
    }
    _functions.blockHeight();
	...
	_functions.initSwiper = function(){
		$('.swiper-container').not('.initialized').each(function(){								  
			...
			swipers['swiper-'+index] = new Swiper('.swiper-'+index,{
				...
				effect: ( $t.is('[data-fade]') ? 'fade' : 'slide' ),
				fade: {
					crossFade: true
				},
	...
	//Smooth Scroll
	var menuItem = $('.navWrapper').find('.hidden-lg').length;
    ...
    $('.menuIcon').on('click', function() {
    	...
    	if ( $('.navWrapper li').hasClass('active') ) $('.imageHoverAnimation').find('.bgImage').eq( $('.navWrapper li.active').index()-menuItem ).toggleClass('active');
    ...
    $('.navWrapper a').hover(
    	function() { //Mouseenter
    		...
    		$('.imageHoverAnimation').find('.bgImage').eq( $(this).parent().index()-menuItem ).addClass('showAnimation');

    		$('.headerDescription .as').removeClass('showAnimation');
    		$('.headerDescription').find('.as').eq( $(this).parent().index()-menuItem ).addClass('showAnimation');
